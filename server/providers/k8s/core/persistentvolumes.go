...
func FetchPersistentVolumes(ctx context.Context, p providers.Provider, teamId primitive.ObjectID) ([]*models.Resource, error) {
	resources := make([]*models.Resource, 0)

	var config v1.ListOptions

	for {
		res, err := p.K8sClient.CoreV1().PersistentVolumes().List(ctx, config)
		if err != nil {
			return nil, err
		}
